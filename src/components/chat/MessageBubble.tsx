       ...
       {!isMine && (
         <div className="w-8 shrink-0 flex items-end mb-0.5">
           {showAvatar ? (
             ...
             >
               {activeChat.avatar_url ? <img src={activeChat.avatar_url} className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full items-center justify-center flex text-[10px] text-white/50">{(activeChat.username?.charAt(0) || '?').toUpperCase()}</div>}
             </div>
           ) : <div className="w-8" />}
         </div>
       )}
