                   ...
                   <div className="w-full h-full relative">
                     {activeChat.participants && activeChat.participants.length > 0 ? (
                       activeChat.participants.slice(0,3).map((p, i, arr) => (
                         <div key={i} className={cn("absolute rounded-full border border-black overflow-hidden bg-white/20", arr.length === 1 ? "inset-0" : arr.length === 2 ? (i === 0 ? "top-0 left-0 w-6 h-6" : "bottom-0 right-0 w-6 h-6") : (i === 0 ? "top-0 left-1/2 -translate-x-1/2 w-5 h-5" : i === 1 ? "bottom-0 left-0 w-5 h-5" : "bottom-0 right-0 w-5 h-5"))}>
                           {p.avatar_url ? <img src={p.avatar_url} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-white/20" />}
                         </div>
                       ))
                     ) : <div className="text-xs">{(activeChat.name?.charAt(0) || '?').toUpperCase()}</div>}
                   </div>
                ) : (
